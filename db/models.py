...
class Ticket(models.Model):
    ...
    def clean(self):
        if not (1 <= self.row <= self.movie_session.cinema_hall.rows and
                1 <= self.row <= self.movie_session.cinema_hall.seats_in_row):
            raise ValidationError("Invalid seat coordinate")
